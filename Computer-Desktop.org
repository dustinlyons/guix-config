#+TITLE: Computer - Desktop Definition
#+STARTUP: content
...
# This script created from Computer-Desktop.org
...
#+BEGIN_SRC sh :noweb-ref initialize-shell-script :noweb-sep ""
echo -e "${GREEN_TERMINAL_OUTPUT}--> [Openbox] Downloading openbox theme...${CLEAR}"
...
(define-module (dl utils)
  ...
  (use-modules (dl computer)
               (dl desktop))

  (define %dl-packages
    (append %dl-packages-computer %dl-packages-desktop))

  (define %dl-dotfiles
    (append %dl-dotfiles-computer %dl-dotfiles-desktop))
