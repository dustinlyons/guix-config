...
(use-package ace-window
  ...
  :custom
    (aw-scope 'frame)
    (aw-keys '(?a ?s ?d ?f ?g ?h ?j ?k ?l)) ;; home row to jump around
...
(defun dl/insert-new-config ()
  "insert the a new config block in my literate config."
       (interactive)
       (insert "*** New Config\n")
       (insert "Description\n\n")
       (insert "**** Initialize\n")
       (insert "#+BEGIN_SRC scheme :noweb-ref initialize-shell-script :noweb-sep \"\"\n")
       (insert "\"\"\n")
       (insert "#+END_SRC\n\n")
       (insert "**** Activate\n")
       (insert "#+BEGIN_SRC scheme :noweb-ref activate-shell-script :noweb-sep \"\"\n")
       (insert "\"\"\n")
       (insert "#+END_SRC\n\n")
       (insert "**** Guix Packages\n")
       (insert "**** Dotfiles Manifest\n\n")
       (insert "#+BEGIN_SRC scheme :noweb-ref dotfiles-manifest :noweb-sep \"\"\n")
       (insert "\"\"\n")
       (insert "#+END_SRC\n\n")
       (insert "**** Dotfiles\n")
       (insert "****** New File"))
...
(dl/leader-keys
  ...
  "cc" '(compile :which-key "compile guix")
  "cr" '(recompile :which-key "recompile guix")
  "d"  '(dl/insert-new-config :which-key "new guix config")
...
(setq compile-command "make -k; make install-home")
